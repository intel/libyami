...
error:
  WARNING ("error parsing \"Reference picture list %u modification\"", list);
  return FALSE;
}
    ...
    BOOL parse_vui_params)
{
  ...
  INFO("final width=%u, height=%u", width, height);
  sps->width = width;
  sps->height = height;
    ...
    H264SEIMessage * sei)
{
  ...
  DEBUG ("SEI message received: payloadType  %u, payloadSize = %u bytes",
