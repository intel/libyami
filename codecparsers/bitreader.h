...
typedef struct {
  ...
  uint32 bit;   /* Bit position in the current byte */
} BitReader;

BitReader * bit_reader_new (const uint8 *data, uint32 size);
void bit_reader_init (BitReader *reader, const uint8 *data, uint32 size);
void bit_reader_free (BitReader *reader);

boolean bit_reader_set_pos (BitReader *reader, uint32 pos);
uint32 bit_reader_get_pos (const BitReader *reader);
uint32 bit_reader_get_remaining (const BitReader *reader);

uint32 bit_reader_get_size (const BitReader *reader);

boolean bit_reader_skip (BitReader *reader, uint32 nbits);
boolean bit_reader_skip_to_byte (BitReader *reader);

boolean bit_reader_get_bits_uint8 (BitReader *reader, uint8 *val, uint32 nbits);
boolean bit_reader_get_bits_uint16 (BitReader *reader, uint16 *val, uint32 nbits);
boolean bit_reader_get_bits_uint32 (BitReader *reader, uint32 *val, uint32 nbits);
boolean bit_reader_get_bits_uint64 (BitReader *reader, uint64 *val, uint32 nbits);

boolean bit_reader_peek_bits_uint8 (const BitReader *reader, uint8 *val, uint32 nbits);
boolean bit_reader_peek_bits_uint16 (const BitReader *reader, uint16 *val, uint32 nbits);
boolean bit_reader_peek_bits_uint32 (const BitReader *reader, uint32 *val, uint32 nbits);
boolean bit_reader_peek_bits_uint64 (const BitReader *reader, uint64 *val, uint32 nbits);
...
static inline void
bit_reader_skip_unchecked (BitReader * reader, uint32 nbits)
{
  reader->bit += nbits;
...
bit_reader_peek_bits_uint##bits##_unchecked (const BitReader *reader, uint32 nbits) \
{ \
  uint##bits ret = 0; \
  const uint8 *data; \
  uint32 byte, bit; \
...
\
static inline uint##bits \
bit_reader_get_bits_uint##bits##_unchecked (BitReader *reader, uint32 nbits) \
{ \
  uint##bits ret; \
  \
...
bit_reader_skip_inline (BitReader * reader, uint32 nbits)
{
...
static inline boolean \
bit_reader_get_bits_uint##bits##_inline (BitReader *reader, uint##bits *val, uint32 nbits) \
{ \
  RETURN_VAL_IF_FAIL (reader != NULL, FALSE); \
...
\
static inline boolean \
bit_reader_peek_bits_uint##bits##_inline (const BitReader *reader, uint##bits *val, uint32 nbits) \
{ \
  RETURN_VAL_IF_FAIL (reader != NULL, FALSE); \
  RETURN_VAL_IF_FAIL (val != NULL, FALSE); \
  RETURN_VAL_IF_FAIL (nbits <= bits, FALSE); \
