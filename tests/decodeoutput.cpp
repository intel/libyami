...
SharedPtr<DecodeOutput> DecodeOutput::create(int renderMode, uint32_t fourcc, const char* inputFile, const char* outputFile)
{
	SharedPtr<DecodeOutput> output;
    switch(renderMode)
    {
        #ifdef __ENABLE_MD5__
        case -2:
            output.reset(new DecoudeOutputMD5(outputFile, inputFile, fourcc));
            break;
        #endif //__ENABLE_MD5__
        case -1:
            output.reset(new DecodeOutputNull());
            break;
        case 0:
            output.reset(new DecodeOutputFile(outputFile, inputFile, fourcc));
            break;
        #ifdef __ENABLE_X11__
        case 1:
            output.reset(new DecodeOutputXWindow());
            break;
        #ifdef __ENABLE_TESTS_GLES__
        case 2:
            output.reset(new DecodeOutputPixelMap());
            break;
        case 3:
            output.reset(new DecodeOutputDmabuf(VIDEO_DATA_MEMORY_TYPE_DRM_NAME));
            break;
        case 4:
            output.reset(new DecodeOutputDmabuf(VIDEO_DATA_MEMORY_TYPE_DMA_BUF));
            break;
